...
func _ready() -> void:
	...
	var hint := Label.new()
	hint.text = "WASD move · SHIFT sprint · SPACE roll · LMB attack · F heavy · R volley · RMB block (tight press = parry) · TAB lock-on · arrows/mouse camera · ESC cursor"
...
func _process(_dt: float) -> void:
	...
	fe_text.text = "%d feathers · %d%% resist" % [int(player.feathers), int(round(player.feather_resist() * 100.0))]
