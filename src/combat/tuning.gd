...
const HITSTOP_DEALT_HEAVY := 0.12   # connect feel: heavies freeze longer than lights
const HITSTOP_CRIT_BONUS := 0.04   # connect feel: crits add freeze on top of the attack's weight
...
const VOLLEY_HIT_SOUND := 5.0
const STAMINA_BREAK_SOUND := 2.0
const LOW_HP_PULSE := 0.25   # low-health feel: below this fraction the screen edges throb   # stamina feel: the break gasp is close-range, your own body spending   # volley feel: the impact carries farther than the loose   # swing feel: heavier swings carry farther (light ~4m, heavy ~5m)   # kill feel: the felling blow shakes on top of the connect answer   # connect feel: crits (riposte/backstab) add this on top   # landing feel: only a real touchdown is audible (tiny drops stay silent)
...
const PERFECT_DODGE_FLASH := 0.30    # the perfect dodge's tell: how long the wind-flash holds (feedback only - still no reward)
...
const HEAL_GLOW_TIME := 0.8           # heal feel: the warmth lingers on the body as it settles
